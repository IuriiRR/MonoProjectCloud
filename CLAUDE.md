...
2. Backend verifies auth token; if no user doc in Firestore → **403 USER_NOT_FOUND**
...
- **Architecture changes** — Update `docs/arc42.md` in the same PR; see its maintenance guide
- **Endpoint changes** — Document in service-specific docs (`docs/users_api.md`, etc.)
- **Schema changes** — Update `docs/firestore_schema.md` and check `firestore.rules` for auth impact
- **Terraform changes** — `tf/README.md` covers GCP setup, permissions, and local credential handling
