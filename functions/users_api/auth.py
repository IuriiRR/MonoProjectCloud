...
def authenticate_request(request) -> Tuple[Optional[str], Optional[Dict[str, Any]], Optional[int]]:
    """
    Returns: (uid, error_payload, http_status)
      - uid is INTERNAL_UID when the request is authenticated via INTERNAL_API_KEY.
      - error_payload/http_status are set only on error.
    """
    if _internal_auth_ok(request.headers):
        return INTERNAL_UID, None, None

    if is_auth_disabled():
        # Dev-only: treat as a fixed local user.
