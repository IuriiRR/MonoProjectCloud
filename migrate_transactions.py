...
def extract_transactions(
    dump_data: List[Dict[str, Any]], currencies: Dict, mcc_codes: Dict
) -> Dict[str, List[Dict]]:
    """
    ...
    for item in dump_data:
        ...
        if model in ["monobank.monotransaction", "monobank.jartransaction"]:
            account_id = fields.get(
                "account"
            )  # The account ID (FK to MonoCard/MonoJar which uses string ID as PK)
...
def fetch_user_accounts(user_id: str) -> List[Dict]:
    ...
    try:
        resp = requests.get(
            f"{ACCOUNTS_API_URL}/users/{user_id}/accounts", headers=HEADERS
        )
        resp.raise_for_status()
...
def migrate_transactions(
    user_id: str, account_id: str, transactions: List[Dict], dry_run: bool = True
):
    if not transactions:
        return
    ...
    print(
        f"  -> Ready to migrate {len(transactions)} transactions for account {account_id} (User: {user_id})"
    )
    ...
    try:
        ...
        url = (
            f"{TRANSACTIONS_API_URL}/users/{user_id}/accounts/{account_id}/transactions"
        )
        ...
        for i in range(0, len(transactions), batch_size):
            ...
            if resp.status_code not in [200, 201]:
                print(
                    f"     [ERROR] Failed to send batch {i//batch_size + 1}: {resp.text}"
                )
            else:
                res_json = resp.json()
                print(
                    f"     [SUCCESS] Batch {i//batch_size + 1}: Processed {res_json.get('processed')} transactions."
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate transactions from Django dump to Functions API."
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Print actions without executing them."
    )
    parser.add_argument(
        "--no-dry-run", dest="dry_run", action="store_false", help="Execute actions."
    )
    ...
    for user in users:
        ...
        for account in accounts:
            ...
            if account_id in json_transactions_map:
                ...
                if missing_txs:
                    print(
                        f"Account {account_id}: Found {len(missing_txs)} missing transactions (out of {len(candidates)} in dump)."
                    )
                    migrate_transactions(
                        user_id, account_id, missing_txs, dry_run=args.dry_run
                    )
